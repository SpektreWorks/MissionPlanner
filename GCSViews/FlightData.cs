...
namespace MissionPlanner.GCSViews
{
    public partial class FlightData : MyUserControl, IActivate, IDeactivate
    {
        ...
        //PWM value to control Nav lights and IR lights separately
        internal static bool navlights_on = false;
        internal static bool irlights_on = false;
        internal static float nav_pwm = 0.0f;
        internal static float ir_pwm = 0.0f;
        internal static float nav_and_IR_pwm = 1000.0f + nav_pwm + ir_pwm;
        internal static bool pld_on = true;  //Payload on by default
        ...
        protected override void OnVisibleChanged(EventArgs e)
        {
            ...
            //Whever the view is changed (i.e., switching between FlightData and FlightPlanning), this method automatically runs.
            //Since the ThemeManager apparently overwrites and changes to the button colors, use this method to recalculate
            //the colors of the buttons. Also, this will allow for an instance of Mission Planner to connect in the middle of 
            ...
            if (this.Visible)
            {
                ...
                {
                    navlights_on = true;
                    BUT_NavLights.BGGradTop = Color.Orange;
                    BUT_NavLights.BGGradBot = Color.Orange;
                }
                else
                {
                    navlights_on = false;
                    BUT_NavLights.BGGradTop = Color.FromArgb(148, 193, 31);
                    BUT_NavLights.BGGradBot = Color.FromArgb(205, 226, 150);
                }

                //IRLights are on if CH13OUT is 1666 or 1999
                if (((MainV2.comPort.MAV.cs.ch13out > (1666 - 50)) && (MainV2.comPort.MAV.cs.ch13out < (1666 + 50))) ||
                    ((MainV2.comPort.MAV.cs.ch13out > (1999 - 50)) && (MainV2.comPort.MAV.cs.ch13out < (1999 + 50))))
                {
                    irlights_on = true;
                    BUT_IRLights.BGGradTop = Color.Orange;
                    BUT_IRLights.BGGradBot = Color.Orange;
                }
                else
                {
                    irlights_on = false;
                    BUT_IRLights.BGGradTop = Color.FromArgb(148, 193, 31);
                    BUT_IRLights.BGGradBot = Color.FromArgb(205, 226, 150);
                }

                //Payload is on if CH14OUT is 1000, off at 2000
                if ((MainV2.comPort.MAV.cs.ch14out > (1000 - 50)) && (MainV2.comPort.MAV.cs.ch14out < (1000 + 50)))
                {
                    pld_on = true;
                    BUT_PLD.BGGradTop = Color.Orange;
                    BUT_PLD.BGGradBot = Color.Orange;
                }
                else
                {
                    pld_on = false;
                    BUT_PLD.BGGradTop = Color.FromArgb(148, 193, 31);
                    BUT_PLD.BGGradBot = Color.FromArgb(205, 226, 150);
                }
            }
        }
        ...
        private void BUT_ARM_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                //If ans is true here, doArm command was successful. Assume the current state is the opposite of isitarmed.
        ...
        private void FlightData_ParentChanged(object sender, EventArgs e)
        {
            ...
            // plane 3.7 and below with airspeed, uses ARSPD_ENABLE:
                     ...
                     (MainV2.comPort.MAV.param.ContainsKey("TRIM_ARSPD_CM") &&
                      ...
                      && (float) MainV2.comPort.MAV.param["ARSPD_USE"] == 1))
            {
                if (modifyandSetSpeed.Enabled == false)
                {
            ...
            try
            {
                ...
                {
                    float rad = (float)MainV2.comPort.MAV.param["WP_LOITER_RAD"] * CurrentState.multiplierdist;
                    if ( rad > 0 )
                    {
                    ...
                    modifyandSetLoiterRad.Value = (decimal) rad;
                }
            }
            catch
            {
        ...
        private void BUT_NavLights_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (navlights_on)
                {
                    //Turn off Nav Lights
                    nav_pwm = 0.0f;
                    nav_and_IR_pwm = 1000.0f + nav_pwm + ir_pwm;
                    if (!MainV2.comPort.doCommand(MAVLink.MAV_CMD.DO_SET_SERVO, 13, nav_and_IR_pwm, 0, 0, 0, 0, 0))
                    {
                        CustomMessageBox.Show(Strings.CommandFailed, Strings.ERROR);
                    }
                    else
                    {
                        //Command succeeded. Update button.
                        navlights_on = false;
                        BUT_NavLights.BGGradTop = Color.FromArgb(148, 193, 31);
                        BUT_NavLights.BGGradBot = Color.FromArgb(205, 226, 150);
                    }
        ...
        private void BUT_IRLights_Click(object sender, EventArgs e)
        {
            if (!MainV2.comPort.BaseStream.IsOpen)
            {
                CustomMessageBox.Show("Please Connect First");
        ...
        private void BUT_PLD_Click(object sender, EventArgs e)
        {
            if (!MainV2.comPort.BaseStream.IsOpen)
            {
                CustomMessageBox.Show("Please Connect First");
